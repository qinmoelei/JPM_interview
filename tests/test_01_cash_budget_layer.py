...
def test_identity_violation_matches_layer_output():
    ...
    states_prev = tf.constant([[10., 0., 100., 0., 50., 20., 30., 5., 10., 15., 25., 40.]], dtype=tf.float32)
    ...
    C, INV, K, BST, BLT, RE, PIC, AR, AP, INVST, OTHER_A, OTHER_LE = tf.unstack(states_t[0])
    assets = C + INV + K + AR + INVST + OTHER_A
    liab_equity = BST + BLT + AP + PIC + RE + OTHER_LE
    diff = tf.abs(assets - liab_equity)
    rel = float((diff / (tf.abs(assets) + 1e-6)).numpy())
    assert pytest.approx(rel, rel=1e-6, abs=1e-6) == metric_value


def test_identity_violation_zero_for_balanced_state():
    balanced = tf.constant(
        [[[10., 5., 20., 10., 15., 7., 3., 3., 5., 2., 6., 6.]]],
        dtype=tf.float32,
    )
